<template>
  <div class="app-container" v-loading="loading">
    ...
    <el-dialog title="发货" :visible.sync="dialogFAHUOVisible" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-form ref="fahuoPopForm" :rules="rulesFAHUO" :model="fahuoPushData" label-position="left" label-width="100px">
        <el-form-item label="快递公司" prop="expressCompanyId">
          <el-col :span="7">
            <el-select v-model="fahuoPushData.expressCompanyId" filterable placeholder="请选择" style="width:100%">
              <el-option v-for="item in expressCompanies" v-bind:key="item.id" :label="item.name" :value="item.id" />
            </el-select>
          </el-col>
        </el-form-item>
        <el-form-item v-if="fahuoPushData.expressCompanyId == 1" label="快递单" prop="createExpressOrder">
          <el-radio-group v-model="fahuoPushData.createExpressOrder" @change="createExpressOrderChange">
            <el-radio :label="false">手动输入运单号</el-radio>
            <el-radio :label="true">创建电子面单</el-radio>
          </el-radio-group>
        </el-form-item>        
        <el-form-item v-if="fahuoPushData.expressCompanyId != 1 || !fahuoPushData.createExpressOrder" label="快递单号" prop="number" >
          <el-col :span="7">
        ...
        <el-form-item v-if="fahuoPushData.expressCompanyId == 1 && fahuoPushData.createExpressOrder" label="门店名称" prop="depotName">
          <el-col :span="7">
            <el-select v-model="fahuoPushData.depotName" placeholder="请选择" style="width:100%">
              <el-option v-for="item in pygjsdDepots" :key="item.name" :label="item.name" :value="item.name" />
            </el-select>
          </el-col>
        </el-form-item>
        <el-form-item v-if="fahuoPushData.expressCompanyId == 1 && fahuoPushData.createExpressOrder" label="选择线路" prop="routeName">
          <el-col :span="7">
            <el-select v-model="fahuoPushData.routeName" placeholder="请选择" style="width:100%">
              <el-option v-for="item in pygjsdRoutes" :key="item.name" :label="item.name" :value="item.name" />
            </el-select>
          </el-col>
        </el-form-item>
        <el-form-item v-if="fahuoPushData.expressCompanyId == 1 && fahuoPushData.createExpressOrder" label="选择省份" prop="province">
          <el-col :span="7">
            <el-select v-model="fahuoPushData.province" placeholder="请选择" style="width:100%" @change="provinceChange">
              <el-option v-for="(item, index) in pygjsdRegion" :key="item.province" :label="item.province" :value="index" />
            </el-select>
          </el-col>
        </el-form-item>
        <el-form-item v-if="fahuoPushData.expressCompanyId == 1 && fahuoPushData.createExpressOrder" label="选择城市" prop="city">
          <el-col :span="7">
            <el-select v-model="fahuoPushData.city" placeholder="请选择" style="width:100%">
              <el-option v-for="item in pygjsdRegionCities" :key="item" :label="item" :value="item" />
            </el-select>
          </el-col>
        </el-form-item>
      </el-form>
...
import { pygjsdDepots, pygjsdRoutes, pygjsdRegion } from '@/api/express'
  ...
  methods: {
    ...
    fahuoSave(){
      this.$refs['fahuoPopForm'].validate((valid) => {
        if (valid) {
          this.fahuoPushData.province = this.fahuoPushData.provinceName
    ...
    createExpressOrderChange(createExpressOrder){
      if (!createExpressOrder) {
        return
      }
      this._pygjsdDepots()
      this._pygjsdRoutes()
    ...
    async _pygjsdDepots(){
      const res = await pygjsdDepots()
      if (res.code == 0) {
        this.pygjsdDepots = res.data.data
      }
    },
    async _pygjsdRoutes(){
      const res = await pygjsdRoutes()
      if (res.code == 0) {
        this.pygjsdRoutes = res.data.data
      }
    },
    async _pygjsdRegion(){
      const res = await pygjsdRegion()
      if (res.code == 0) {
        this.pygjsdRegion = res.data.data
      }
    },
    async provinceChange(provinceIndex){
      const pygjsdRegionCities = []
      this.pygjsdRegion[provinceIndex].city.split(",").forEach(ele => {
        pygjsdRegionCities.push(ele.split('_')[0])
      })
      this.fahuoPushData.provinceName = this.pygjsdRegion[provinceIndex].province
      this.pygjsdRegionCities = pygjsdRegionCities
    },
  }
}
</script>
