  ...
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column label="库存/状态" align="center">
        <template slot-scope="scope">
          {{scope.row.stores}}<br>
          <el-tag v-if="scope.row.vetStatus == 0" type="info">待审核</el-tag>
          <el-tag v-if="scope.row.vetStatus == 1" type="success">审核通过</el-tag>
          <el-tag v-if="scope.row.vetStatus == 2" type="danger">审核不通过</el-tag>
          <div style="margin-top:5px;">
            <el-tag v-if="scope.row.status == 0" type="success">上架</el-tag>
            <el-tag v-if="scope.row.status == 1" type="danger">下架</el-tag>
          </div>
          <div style="margin-top:5px;">
            <el-tag v-if="!scope.row.hidden" type="success">默认展示</el-tag>
            <el-tag v-if="scope.row.hidden" type="danger">默认隐藏</el-tag>
          </div>
        </template>
      </el-table-column>
    ...
    <el-button style='margin-top:20px' type="success" size="mini" @click="batchVetStatus(1)">审核通过</el-button>
    <el-button style='margin-top:20px' type="warning" size="mini" @click="batchVetStatus(2)">审核不通过</el-button>
